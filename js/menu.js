	...
	create: function( options ) {
		return (function(){
			...
			function setupWrapper() {
				...
				// position
				menu.addClass( dom.wrapper, 'menu-' + config.position );

				originalStyles.wrapper = dom.wrapper.style.cssText;

				dom.wrapper.style[ menu.prefix( 'perspective' ) ] = '800px';
				dom.wrapper.style[ menu.prefix( 'perspectiveOrigin' ) ] = contentsTransformOrigin;
			}
			...
			function setupCover() {
				...
				if( supports3DTransforms ) {
					dom.cover.style[ menu.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
			...
			function setupMenu() {
				...
				if( supports3DTransforms ) {
					style[ menu.prefix( 'transform' ) ] = menuTransformClosed;
					style[ menu.prefix( 'transformOrigin' ) ] = menuTransformOrigin;
					style[ menu.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					menu.extend( style, menuStyleClosed );
				}
			}
			...
			function setupContents() {
				...
				if( supports3DTransforms ) {
					style[ menu.prefix( 'transform' ) ] = contentsTransformClosed;
					style[ menu.prefix( 'transformOrigin' ) ] = contentsTransformOrigin;
					style[ menu.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					style.position = style.position.match( /relative|absolute|fixed/gi ) ? style.position : 'relative';
					menu.extend( style, contentsStyleClosed );
				}
			}
			...
			function bindEvents() {
				...
				if( 'ontouchstart' in window ) {
					if( config.touch ) {
						menu.bindEvent( document, 'touchstart', onTouchStart );
						menu.bindEvent( document, 'touchend', onTouchEnd );
					}
					else {
						menu.unbindEvent( document, 'touchstart', onTouchStart );
						menu.unbindEvent( document, 'touchend', onTouchEnd );
					}
				}

				if( config.mouse ) {
					menu.bindEvent( document, 'mousedown', onMouseDown );
					menu.bindEvent( document, 'mouseup', onMouseUp );
					menu.bindEvent( document, 'mousemove', onMouseMove );
				}
				else {
					menu.unbindEvent( document, 'mousedown', onMouseDown );
					menu.unbindEvent( document, 'mouseup', onMouseUp );
					menu.unbindEvent( document, 'mousemove', onMouseMove );
				}
			}
			...
			function open() {
				if( !isOpen ) {
					...
					menu.addClass( dom.wrapper, 'menu-active' );
					...
					if( supports3DTransforms ) {
						...
						menu.bindEventOnce( dom.wrapper, 'transitionend', function() {
							menu.dispatchEvent( dom.menu, 'opened' );
						} );
					...
					menu.dispatchEvent( dom.menu, 'open' );
				}
			}
			...
			function close() {
				if( isOpen ) {
					...
					menu.removeClass( dom.wrapper, 'menu-active' );

					// Use transforms and transitions if available...
					if( supports3DTransforms ) {
						// 'webkitAnimationEnd oanimationend msAnimationEnd animationend transitionend'
						menu.bindEventOnce( dom.wrapper, 'transitionend', function() {
							menu.dispatchEvent( dom.menu, 'closed' );
						} );
					...
					// ...fall back on JS animation
						...
						coverAnimation && coverAnimation.stop();
						coverAnimation = menu.animate( dom.cover, { opacity: 0 }, 500, function() {
							dom.cover.style.visibility = 'hidden';
							menu.dispatchEvent( dom.menu, 'closed' );
						} );
					}
					menu.dispatchEvent( dom.menu, 'close' );
				}
			}

			/**
			 * Unbinds menu and resets the DOM to the state it
			 * was at before menu was initialized.
			 */
			function destroy() {
			...
			function onTouchStart( event ) {
				...
				menu.bindEvent( document, 'touchmove', onTouchMove );
			}
			...
			function onTouchEnd( event ) {
				menu.unbindEvent( document, 'touchmove', onTouchMove );
			...
			return {
				...
				addEventListener: function( type, listener ) {
					...
					dom.menu && menu.bindEvent( dom.menu, type, listener );
				},
				removeEventListener: function( type, listener ) {
					dom.menu && menu.unbindEvent( dom.menu, type, listener );
				}
			};
